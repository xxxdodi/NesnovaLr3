...
export default {
  ...
  actions: {
    fetchItems: async ({ commit }) => {
      const response = await api.subjectsfiltred();
    ...
    addItem: async ({ commit }, { subject, description, hours, teacher }) => {
      const item = await api.add({ subject, description, hours, teacher })
      commit('setItem', item)
    },
    updateItem: async ({ commit }, { id, subject, description, hours, teacher }) => {
      const item = await api.update({ id, subject, description, hours, teacher });
