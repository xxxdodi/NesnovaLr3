...
export default {
  ...
  actions: {
    ...
    addItem: async ({ commit }, { teacher, department }) => {
      const item = await api.add({ teacher, department });
      commit('addItem', item);
    },
    updateItem: async ({ commit }, { id, teacher, department }) => {
      const item = await api.update({ id, teacher, department });
